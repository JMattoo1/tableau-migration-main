...
def find_tables_from_twb(soup, folderPath, wbPath,tdsSrcName,is_published):
    ...
    if not is_published :
        ...
        for table in tables:
            ...
            # 08052021
            # add port, server, schema, filename
            portTag = connectionTag.get('port')
            serverTag = connectionTag.get('server')
            schemaTag = connectionTag.get('schema')
            
            from os.path import basename
            fileNameTag = connectionTag.get('filename')

            ports.append(portTag if portTag else "")
            servers.append(serverTag if serverTag else "")
            schemas.append(schemaTag if schemaTag else "")
            fileNames.append(basename(fileNameTag) if fileNameTag else "")
    ...
    tableData = pd.DataFrame({
        ...
        "tableRefName": tableRefNames, 
        "tableTypes": tableTypes, 
        "jointTypes": jointTypes, 
        "queries": queries, 
        "connectionStrings": connectionStrings,
        "datasourceType":datasourceType,
        "listTables":tableQueries,
        "ports": ports,
        "servers": servers,
        "schemas": schemas,
        "filename": fileNames
...
def parse_column_tag_twb(soup, folderPath, wbPath,tdsSrcName, contents):
    folderNames = []
    workBooks = []
    datasourceNames = []
    fieldNames = []
    ...
    for column in columns:
        ...
        if column.findParent("datasource-dependencies") is None:
            ...
            if column.get("role") is not None:
                if column.find("calculation") is None:
                    if fieldNames[-1] is None:
                        formulaClass.append(None)
                        formulas.append(None)
                        columnTypes.append("BASE")
                    else:
                        if fieldNames[-1].lower() != "user filter":
                            formulaClass.append(None)
                            formulas.append(None)
                            columnTypes.append("BASE")
                        else:
                            ...
                            raw_soup = BeautifulSoup(contents,'html.parser').find_all(attrs={"datatype":"string", "name":"datasource"})
                            found_flag = 0
                            for cd in raw_soup:
                                cdata = BeautifulSoup(cd.contents[1], 'html.parser')
                                cdata_cols = cdata.find_all("column")
                                if found_flag:
                                    break
                                for cdata_col in cdata_cols:
                                    caption = cdata_col.get("caption") 
                                    if  caption is not None:
                                        if caption.lower() == "user filter":
                                            formulaClass.append(cdata_col.find("calculation").get("class"))
                                            formulas.append(cdata_col.find("calculation").get("formula"))
                                            columnTypes.append("BASE")
                                            found_flag = 1
                                            break
...
def parse_dashboards(soup, folderPath, wbPath):
    dbs = soup.find_all("dashboard")
    folderNames = []
    workBooks = []
    dashboards = []
    ...
    for db in dbs:
        folderNames.append(folderPath)
        workBooks.append(wbPath)
        dashboards.append(db.get("name"))

    dashboardData = pd.DataFrame({"FolderName": folderNames, "twb": workBooks, 
    "dashboards": dashboards})
    return dashboardData

def parse_datasource(soup, folderPath, wbPath):
    worksheets = soup.find_all("worksheet")
    worksheets = [s for s in worksheets if s.get("name") is not None]
    ds_relationships = soup.find_all("datasource-relationship")
    folderNames = []
    workBooks = []
    sources = []
    targets = []
    ...
    for dsr in ds_relationships:
        folderNames.append(folderPath)
        workBooks.append(wbPath)

        source = dsr.get("source")
        target = dsr.get("target")
        pairs.add((source, target))
    if not ds_relationships:
        for sid in sourceNames.keys():
            folderNames.append(folderPath)
            workBooks.append(wbPath)
            pairs.add((sid, sid))
    ...
    # try:
    #     for s, t in pairs:
    #         sources.append(sourceNames[s])
    #         targets.append(sourceNames[t])
    # except Exception as e:
    #     import traceback
    #     traceback.print_exc()
    #     print(f'workbook: {wbPath} {sourceNames}')

    for s, t in pairs:
        sources.append(sourceNames[s])
        targets.append(sourceNames[t])

    datasourceData = pd.DataFrame({"FolderName": folderNames, "twb": workBooks, 
