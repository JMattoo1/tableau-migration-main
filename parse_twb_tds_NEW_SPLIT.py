...
import re
from config import inFolderName, inFolderPath, outFilePath
...
sys.argv.append(inFolderName)
folder = inFolderPath + sys.argv[1]
outFileName = outFilePath + sys.argv[1] + ".xlsx"
# subFolder =  os.listdir(folder)
# # subFolder = [os.path.join(folder, f) for f in subFolder]
...
for f in subFolder:
    ...
    for i in tqdm(range(len(workBooks))):
        path = wbPaths[i]
        ...
        for sourceName, sourceType in zip(sourceNames, sourceTypes):
            ...
            if sourceType == "sqlproxy":
                ...
                if os.path.exists(tdsPath):
                    ...
                    tableList.append(tableData)
                    #print(tableData)
                    columnDataOne = ps.parse_column_tag_twb(soup=soup, folderPath=f, wbPath=workBook,tdsSrcName = sourceName, contents=contents)
                    # print(columnDataOne)
                    columnListOne.append(columnDataOne)
        ...
        dashboardData = ps.parse_dashboards(soup=soup, folderPath=f, wbPath=workBook)
        dashboardList.append(dashboardData)
        datasourceData = ps.parse_datasource(soup=soup, folderPath=f, wbPath=workBook)
        datasourceList.append(datasourceData)
...
dashboardData = pd.concat(dashboardList)
datasourceData = pd.concat(datasourceList)
...
# 28022023 : Add columns (dax_formula = equivalent formula in DAX language; require_review =  if futher review is required) for Column Sheet
from convert_expression import instance as ce
tqdm.pandas()
converter = ce.createInstances(columnData)
tableauResult = lambda x: converter.convert(x['formula'], x['datatype'], x['twb'], x['tds']) if x['column_type'] == "DERIVED" else (np.NaN, 0)
columnData[['dax_formula',"require_review"]] = pd.DataFrame(columnData.progress_apply(tableauResult, axis=1).tolist(), index=columnData.index)
...
with pd.ExcelWriter(outFileName) as writer:
    ...
    dashboardData.sort_values(['FolderName','twb'],ascending=[True,True]).to_excel(writer, sheet_name="dashboard", index=False)
    datasourceData.sort_values(['FolderName','twb'],ascending=[True,True]).to_excel(writer, sheet_name="datasources", index=False)
